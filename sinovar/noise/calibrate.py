...
# Project a batch of images, one random angle each; the shift is shared across the
# batch because a translation only adds a phase ramp and leaves |rfft|^2 unchanged.
_project_lines = jax.vmap(project_sinogram, in_axes=(0, None, 0))
...
def estimate_projected_line_noise_variance(
    ...
    :func:`project_sinogram`. That projector samples the image with bilinear
    interpolation and clips rays at the image border, which both reshapes the
    noise spectrum and attenuates it (to ~60% of a clean line integral for a
    generic angle). Rather than model that transfer analytically, this pushes
    synthetic noise carrying the estimated PSD through the *same* projector and
    measures the resulting per-frequency variance --- exactly the ``sigma2`` the
    distance kernel needs, with no scale factor left to guess.
    ...
    # of the default-norm rfft frequency by frequency.
    angles = jax.random.uniform(key_angle, (n_samples, 1), maxval=2 * jnp.pi)
    shift = jnp.zeros((2,), dtype=images.dtype)
    lines = _project_lines(images, shift, angles)[:, 0, :]   # (n_samples, box)
